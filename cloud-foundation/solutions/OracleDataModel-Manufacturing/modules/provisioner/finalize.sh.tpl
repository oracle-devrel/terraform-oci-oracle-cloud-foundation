...
mkdir DISCRT_WITH_PROCESS_MANUFACTURING
cd DISCRT_WITH_PROCESS_MANUFACTURING
echo "-- Retrive the original bar file from the shared object storage"
wget -q https://objectstorage.us-ashburn-1.oraclecloud.com/p/n-esoXVRbrXTE3ZccuZflHLAR3J2_nCLOBqGFRYrDD3oOCzJmqX2HoErShBarB5y/n/oradbclouducm/b/bucket-20200907-1650/o/3feb2023_data_model_manufacturing.bar
echo "-- Fixing the bar file"
unzip -qq 3feb2023_data_model_manufacturing.bar
rm -rf 3feb2023_data_model_manufacturing.bar
echo "-- Modify the links for the picture using the current hostname of OAC, tenancy name and region key"
perl -pi.back -e "s/DataModelsManufacturinn-apaccpt03-ia.analytics.ocp.oraclecloud.com/$Oracle_Analytics_Instance_Name-$Tenancy_Name-$home_region_for_oac.analytics.ocp.oraclecloud.com/g"  content/catalog/root/shared/discrt_manufacturing/_portal/discrt_manufacturing_dashboard/page+1
echo "-- Create new bar file for OAC with the custom configuration"
zip -fz DISCRT_WITH_PROCESS_MANUFACTURING_modified.bar -r -qq *
echo "-- Upload the snapshot to the object storage in our tenancy"
oci os object put -ns $Tenancy_Name -bn $bucket --force --file DISCRT_WITH_PROCESS_MANUFACTURING_modified.bar > /dev/null
cd ..
rm -rf DISCRT_WITH_PROCESS_MANUFACTURING
...
cat restore_mysnapshot.json | jq --arg id $id '{"snapshot": { "id":$id, "password":"Admin123"}}' > restore_mysnapshot_new.json
echo ""
...
oci os object delete --bucket-name $bucket --force --object-name DISCRT_WITH_PROCESS_MANUFACTURING_modified.bar
echo "-- Restore Done!"
