...
resource "null_resource" "remote-exec" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "curl -X PUT -u 'ADMIN:${var.database_admin_password}' '${var.atp_url}/admin/soda/latest/fruit'",
      "curl -X POST -u 'ADMIN:${var.database_admin_password}' -H 'Content-Type: application/json' --data '{\"name\":\"orange\", \"count\":42}' '${var.atp_url}/admin/soda/latest/fruit'",
      "curl -X POST -u 'ADMIN:${var.database_admin_password}' -H 'Content-Type: application/json' --data '{\"name\":\"orange\"}' '${var.atp_url}/admin/soda/latest/fruit?action=query' >> /usr/share/nginx/html/index.html",
